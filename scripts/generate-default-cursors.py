...
def create_pointer_cursor(size=32):
    ...
    # Index pointé
    draw.rectangle(
        [size // 2 - 3, size // 4, size // 2 + 3, size // 2],
        fill=(255, 220, 180, 255),
        outline=(0, 0, 0, 255),
        width=2,
    )
    draw.ellipse(
        [size // 2 - 4, size // 4 - 8, size // 2 + 4, size // 4 + 4],
        fill=(255, 220, 180, 255),
        outline=(0, 0, 0, 255),
        width=2,
    )
...
def create_loading_cursor(size=32, frames=12):
    ...
    for i in range(frames):
        ...
        for j in range(12):
            ...
            draw.ellipse(
                [x - size_dot, y - size_dot, x + size_dot, y + size_dot],
...
def main():
    ...
    output_dir = os.path.join(
        os.path.dirname(os.path.dirname(__file__)), "assets", "cursors"
    )
    ...
    save_gif(
        animated_frames, os.path.join(output_dir, "animated-cursor.gif"), duration=100
    )
    ...
    save_gif(
        animated_hd, os.path.join(output_dir, "animated-cursor-hd.gif"), duration=100
    )
