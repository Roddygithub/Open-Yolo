    ...
    try:
        ...
        if size > 32:  # Uniquement pour les icônes suffisamment grandes
            ...
            shadow_layer = shadow_layer.filter(
                ImageFilter.GaussianBlur(radius=size // 32)
            )
        ...
        try:
            ...
            draw.text((text_x, text_y), text, font=font, fill=THEME_COLORS["text_dark"])
            ...
            if size > 64:
                ...
                for y in range(size // 4):
                    ...
                    reflection_draw.line([(0, y), (size, y)], fill=alpha, width=1)
...
def generate_icon_set() -> bool:
    ...
    try:
        ...
        icon_sizes = [16, 22, 24, 32, 48, 64, 96, 128, 192, 256, 384, 512, 1024]
        ...
        try:
            ...
            if (
                not target_icon.exists()
                and (
                    ICONS_DIR / "hicolor" / "512x512" / "apps" / "OpenYolo.png"
                ).exists()
            ):
                img = Image.open(
                    ICONS_DIR / "hicolor" / "512x512" / "apps" / "OpenYolo.png"
