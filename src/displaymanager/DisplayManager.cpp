...
struct DisplayManager::Impl {
    ...
    void updateDisplays() {
        ...
        for (int i = 0; i < res->noutput; i++) {
            ...
            if (!output_info) {
                std::cerr
                    << "Erreur lors de la récupération des informations de sortie pour l'écran "
                    << i << std::endl;
                continue;
            }
            ...
            info.scale =
                1.0f;  // Par défaut, à moins qu'on ne trouve une meilleure façon de l'obtenir
    ...
    const std::vector<DisplayInfo>& getDisplays() const { return displays; }

    const DisplayInfo* getPrimaryDisplay() const { return primaryDisplay; }
    ...
    bool isFollowingMouse() const { return followMouse; }
...
bool DisplayManager::initialize() {
    try {
        ...
        std::cerr << "Erreur lors de l'initialisation du gestionnaire d'écrans: " << e.what()
                  << std::endl;
        return false;
    }
}

void DisplayManager::updateDisplays() { pImpl->updateDisplays(); }
...
void DisplayManager::setFollowMouse(bool enable) { pImpl->setFollowMouse(enable); }

bool DisplayManager::isFollowingMouse() const { return pImpl->isFollowingMouse(); }

void DisplayManager::setWindowSize(int width, int height) { pImpl->setWindowSize(width, height); }

void DisplayManager::setFullscreen(bool fullscreen) { pImpl->setFullscreen(fullscreen); }

void DisplayManager::beginFrame() { pImpl->beginFrame(); }

void DisplayManager::endFrame() { pImpl->endFrame(); }
