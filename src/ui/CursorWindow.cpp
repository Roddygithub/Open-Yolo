...
CursorWindow::CursorWindow()
    : m_mainBox(Gtk::ORIENTATION_VERTICAL, 6),
      ...
      m_saveProfileBtn("Save Profile"),
      m_importBtn("Import Profile"),
      m_exportBtn("Export Profile"),
    ...
    if (!m_cursorManager.initialize()) {
        Gtk::MessageDialog dialog(*this, "Failed to initialize cursor manager", false,
                                  Gtk::MESSAGE_ERROR);
        dialog.run();
        return;
    }
...
void CursorWindow::setup_ui() {
    ...
    m_profileBox.pack_start(m_importBtn, Gtk::PACK_SHRINK);
    m_profileBox.pack_start(m_exportBtn, Gtk::PACK_SHRINK);
...
void CursorWindow::setup_signals() {
    ...
    m_browseBtn.signal_clicked().connect(sigc::mem_fun(*this, &CursorWindow::on_browse_clicked));

    m_applyBtn.signal_clicked().connect(sigc::mem_fun(*this, &CursorWindow::on_apply_clicked));

    m_saveProfileBtn.signal_clicked().connect(
        sigc::mem_fun(*this, &CursorWindow::on_save_profile_clicked));

    m_importBtn.signal_clicked().connect(sigc::mem_fun(*this, &CursorWindow::on_import_clicked));

    m_exportBtn.signal_clicked().connect(sigc::mem_fun(*this, &CursorWindow::on_export_clicked));
...
void CursorWindow::load_profiles() {
    ...
    for (const auto& profile : openyolo::ProfileManager::instance().getProfiles()) {
        m_profileCombo.append(profile.name, profile.name);
    }
...
void CursorWindow::on_profile_changed() {
    ...
    if (active_id == "system") {
        ...
        //     Gtk::MessageDialog dialog(*this, "Failed to load profile", false,
        //     Gtk::MESSAGE_ERROR); dialog.set_secondary_text("Could not load the selected
        //     profile."); dialog.run();
        // }
    }
}
...
void CursorWindow::on_save_profile_clicked() {
    ...
    if (dialog.run() == Gtk::RESPONSE_ACCEPT) {
        ...
        if (!profileName.empty()) {
            openyolo::ProfileManager::instance().createProfile(profileName);
            load_profiles();  // Recharger la liste des profils
        }
    }
}

void CursorWindow::on_import_clicked() {
    Gtk::FileChooserDialog dialog("Import Profile", Gtk::FILE_CHOOSER_ACTION_OPEN);
    dialog.set_transient_for(*this);
    ...
    dialog.signal_response().connect([this, &dialog](int response_id) {
        if (response_id == Gtk::RESPONSE_ACCEPT) {
            std::string filename = dialog.get_filename();
            if (auto err = openyolo::ProfileManager::instance().importProfile(filename)) {
                Gtk::MessageDialog error_dialog(*this, "Failed to import profile", false,
                                                Gtk::MESSAGE_ERROR);
                error_dialog.set_secondary_text(err.message());
                error_dialog.run();
            } else {
                Gtk::MessageDialog info_dialog(*this, "Profile Imported", false, Gtk::MESSAGE_INFO);
                info_dialog.set_secondary_text("Profile imported from " + filename);
                info_dialog.run();
                load_profiles();
            }
        }
...
void CursorWindow::on_export_clicked() {
    Gtk::FileChooserDialog dialog("Export Profile", Gtk::FILE_CHOOSER_ACTION_SAVE);
    dialog.set_transient_for(*this);
    ...
    dialog.signal_response().connect([this, &dialog](int response_id) {
        if (response_id == Gtk::RESPONSE_ACCEPT) {
            std::string filename = dialog.get_filename();
            Glib::ustring active_id = m_profileCombo.get_active_id();
            if (active_id.empty() || active_id == "system") {
                Gtk::MessageDialog error_dialog(*this, "Cannot export system profile", false,
                                                Gtk::MESSAGE_ERROR);
                error_dialog.run();
                return;
            }
            if (auto err =
                    openyolo::ProfileManager::instance().exportProfile(active_id, filename)) {
                Gtk::MessageDialog error_dialog(*this, "Failed to export profile", false,
                                                Gtk::MESSAGE_ERROR);
                error_dialog.set_secondary_text(err.message());
                error_dialog.run();
            } else {
                Gtk::MessageDialog info_dialog(*this, "Profile Exported", false, Gtk::MESSAGE_INFO);
                info_dialog.set_secondary_text("Profile exported to " + filename);
                info_dialog.run();
            }
        }
        dialog.hide();
