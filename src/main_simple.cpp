...
#define OY_FATAL(msg)   { std::cerr << "[FATAL] " << msg << std::endl; std::exit(EXIT_FAILURE); }
...
void renderLoop() {
    try {
        ...
        while (g_running && g_cursorManager) {
            ...
            // Note: La méthode update() n'existe pas dans CursorManager, nous l'utilisons comme exemple
            // Si vous avez besoin d'une méthode de mise à jour, vous devrez l'implémenter dans CursorManager
            // Mettre à jour le rendu ici
            if (g_cursorManager) {
