...
void input::WaylandBackend::setWindow(Gtk::Window* window) {
    ...
    std::cout << "[WaylandBackend] Fenêtre associée : " << (this->m_window ? "OK" : "NULL")
              << std::endl;
...
bool WaylandBackend::onKeyPressed(GdkEventKey* event) {
    ...
    std::cout << "[WaylandBackend] Touche pressée : " << gdk_keyval_name(event->keyval)
              << " (keycode: " << event->keyval << ")" << std::endl;
...
void input::WaylandBackend::onSessionCreated(Glib::RefPtr<Gio::AsyncResult>& result) {
    ...
    try {
        ...
        std::cerr << "[WaylandBackend] Erreur lors de la création de la session: " << error.what()
                  << std::endl;
...
bool input::WaylandBackend::initialize() {
    ...
    if (!gdkDisplay || !GDK_IS_WAYLAND_DISPLAY(gdkDisplay->gobj())) {
        this->handleError(
            "Le backend Wayland ne peut être initialisé que sur un affichage Wayland.");
        return false;
    }
    ...
    Gio::DBus::Proxy::create(
        this->m_dbusConnection, input::DBUS_SERVICE, input::DBUS_PATH, input::DBUS_INTERFACE,
    ...
    try {
        ...
        if (!this->isProxyValid()) {
            throw std::runtime_error(
                "Le service de portail XDG n'est pas disponible. Vérifiez que xdg-desktop-portal "
                "est installé et en cours d'exécution.");
        }
        LOG_INFO("Proxy D-Bus pour GlobalShortcuts créé. Fourni par: " +
                 this->m_proxy->get_name_owner());
...
void input::WaylandBackend::closeSession() {
    ...
    if (!this->isProxyValid()) {
        ...
        try {
            this->m_proxy->call("CloseSession",
                                Glib::VariantContainerBase::create_tuple(
                                    {Glib::Variant<Glib::ustring>::create(this->m_sessionHandle)}));
        } catch (const std::exception& e) {
            this->handleError(
                "Erreur (ignorée) lors de la demande de fermeture de la session D-Bus", &e);
        }
    }
...
bool input::WaylandBackend::registerShortcut(const std::string& name,
    ...
    try {
        ...
        auto params = Glib::VariantContainerBase::create_tuple(
            {Glib::Variant<Glib::ustring>::create("org.freedesktop.portal.Desktop"),
             ...
             Glib::wrap(g_variant_new_parsed(
                 "(@a(sa{sv})a{sv})",
                 g_variant_new_parsed("[('%s', {'shortcut': <%s>})]", name.c_str(),
...
void input::WaylandBackend::scheduleReconnect() {
    ...
    this->m_updateTimer = Glib::signal_timeout().connect(
        sigc::bind_return(sigc::mem_fun(*this, &WaylandBackend::initialize),
                          false  // Ne pas répéter
                          ),
        input::RETRY_DELAY_SEC * 1000  // Convertir en millisecondes
    );
}
void input::WaylandBackend::onSessionShortcutActivated(
    const Glib::ustring& sender_name, const Glib::ustring& signal_name,
    const Glib::VariantContainerBase& parameters) {
    if (signal_name != "GlobalShortcutFired" || parameters.get_n_children() < 2)
        return;
    try {
        ...
        if (session_handle.get() != this->m_sessionHandle)
            return;
