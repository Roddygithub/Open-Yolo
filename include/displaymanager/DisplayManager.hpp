...
struct DisplayInfo {
    ...
    DisplayInfo()
        : id(-1), x(0), y(0), width(0), height(0), scale(1.0f), isPrimary(false), ptr(nullptr) {}

    // Constructeur avec paramètres
    DisplayInfo(int32_t id, int32_t x, int32_t y, uint32_t w, uint32_t h, float s = 1.0f,
                bool primary = false, const std::string& n = "")
        : id(id),
          x(x),
          y(y),
          width(w),
    ...
    bool operator==(const DisplayInfo& other) const {
        return id == other.id && x == other.x && y == other.y && width == other.width &&
               height == other.height && isPrimary == other.isPrimary;
    }

    // Vérifie si un point est contenu dans cet écran
    bool contains(int32_t px, int32_t py) const {
        return px >= x && px < static_cast<int32_t>(x + width) && py >= y &&
